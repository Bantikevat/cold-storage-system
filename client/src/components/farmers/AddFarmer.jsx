...
const AddFarmer = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/api/farmers/add", formData); // Using axios
      ...
      setNotification({ 
        show: true, 
        type: 'error', 
        message: `❌ Error adding farmer: ${error.response?.data?.message || error.message || 'Please try again.'}` 
      });
    } finally {
      setLoading(false);
    }
